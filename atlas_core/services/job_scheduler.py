...
class JobScheduler:
    ...
    def register_active(self, job_id: str, policy: str, pid: int) -> bool:
        ...
        with self._lock:
            ...
            if policy == "large":
                large_active = sum(
                    1 for v in self._active.values() if v.get("policy") == "large"
                )
                if large_active < getattr(
                    self.policy, "max_concurrent_large_documents", 1
                ):
            ...
            # standard
            standard_active = sum(
                1 for v in self._active.values() if v.get("policy") == "standard"
            )
    ...
    def submit(self, job_file: str | Path) -> str:
        ...
        tier = (
            js.get("classification_policy") or js.get("processing_class") or "standard"
        )
        tier = str(tier)
        now = time.time()
        ...
        with self._lock:
            ...
            very_large_active = any(
                v.get("policy") == "very_large" for v in self._active.values()
            )
            ...
            if tier == "standard":
                ...
                if standard_active < getattr(
                    self.policy, "standard_job_concurrency", 1
                ):
    ...
    def worker_finished(self, job_id: str) -> None:
        ...
        with self._lock:
            ...
            self._emit_event(
                "completed",
                job_id,
                removed.get("policy") if removed else "unknown",
                0.0,
            )
    ...
    def _admit_queued(self) -> None:
        ...
        while (
            self._queues.get("large")
            and large_active < large_limit
            and not very_large_active
        ):
        ...
        while (
            self._queues.get("standard")
            and sum(1 for v in self._active.values() if v.get("policy") == "standard")
            < std_limit
            and not any(v.get("policy") == "very_large" for v in self._active.values())
        ):
