...
AV-00A addendum unifies bid-package upload policy across the Documents uploader, pending validation, intake validation, background job queueing, help text, and diagnostics with a 200 MB per-file decimal-byte limit, a bounded supported-format list, JSON removed from user-facing bid-package intake, and unchanged ZIP container safeguards.
