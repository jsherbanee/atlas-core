...
- UI-FIX-01 removes the Commercial/Sales primary navigation item, standalone Commercial Workspace route, and combined page after manual review of PR #10 found that the surface duplicated existing product areas and confused tenant workflows.
- Commercial UI ownership is now explicit: Transactions owns commercial documents and workflows; Knowledge owns customers, vendors, manufacturers, and catalog; Reports owns commercial read models; Settings owns appropriate tenant-facing configuration.
- UI-FIX-01 also prevents a selected project from rendering global project selectors, headers, actions, breadcrumbs, or project status context outside Projects. No new primary navigation item was introduced.
- The commercial backend spine, APP-01/API-01 boundaries, repository services, and tenant isolation remain intact. Focused follow-up work is still required within existing product areas, and this remediation does not claim broad UI readiness.
- A future Platform Admin Console is documented as a separate application boundary and is not part of the tenant-facing navigation or this implementation slice.
- MVP-CHK-01 previously validated the Commercial MVP backend end to end across boundary, facade, services, repositories, tenant isolation, reporting, and QuickBooks sync readiness. UI-FIX-01 replaces the removed page tests with route-removal, primary-nav exclusivity, non-Projects context-isolation, and existing-workspace navigation regressions while preserving backend coverage.
...
Sprint T-08 operationalizes Customer Invoice transactions, including invoice draft origins (standalone/project/source-linked), billing-strategy controls and overbilling override diagnostics, issue/revision/export continuity, and invoice-specific sync/payment-status metadata handling.
...
AV-00A addendum unifies bid-package upload policy across the Documents uploader, pending validation, intake validation, background job queueing, help text, and diagnostics with a 200 MiB per-file limit, 1 GiB per-batch limit, 50-file batch limit, a bounded supported-format list, JSON removed from user-facing bid-package intake, accepted-only job creation, and expanded ZIP container safeguards.
...
Sprint A-03 implements controlled external alpha tester onboarding and rollout operations, including tester profile lifecycle controls, onboarding acknowledgements, deterministic scenario assignment/completion tracking, tester-scoped reset/export request intake, platform-admin operations dashboard visibility, and deactivation access enforcement without adding new product features.
Sprint A-04 (Alpha Operations) implements controlled alpha launch and stabilization loop operations, including release records, tester-cohort assignment, feedback triage queue, feedback-to-defect conversion, deterministic defect lifecycle controls (confirmed/retest/verified), tenant-safe visibility filtering, and stabilization release history tracking without introducing product feature expansion.
...
- A-02 controlled alpha deployment and test-operations package complete; continue P-08 readiness planning for attachment security depth within approved roadmap scope.
