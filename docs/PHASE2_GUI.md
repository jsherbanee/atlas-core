...
Atlas Workspace Knowledge Cleanup replaces the custom Knowledge accordion with native disclosure sections, removes redundant Customer sort buttons, and makes the compact Customer selector the primary browse/detail handoff.
Atlas Workspace App-Wide Navigation Refactor replaces widget-owned first-column expanders with a shared controlled accordion so Projects, Transactions, Knowledge, Reports, and Settings allow only one expanded secondary section at a time.
