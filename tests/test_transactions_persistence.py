...
from atlas_core.services.transactions_workspace_service import TransactionsWorkspaceService
...
def _create_project(root: Path, project_id: str) -> tuple[ProjectWorkspaceService, str]:
    ...
    (project_dir / "project.json").write_text(json.dumps({"project_id": project_id, "name": project_id, "client": "ACME"}), encoding="utf-8")
    (project_dir / "metadata.json").write_text(json.dumps({"project_name": project_id, "created_at": _utc_now(), "owner": "ACME"}), encoding="utf-8")
    (project_dir / "workspace.json").write_text(json.dumps({"workspace_id": project_id, "created_at": _utc_now(), "updated_at": _utc_now()}), encoding="utf-8")
...
def test_serialization_round_trip_and_persistence(tmp_path):
    ...
    svc.set_approval_state(document_id=est.document_id, approval_state=ApprovalState.APPROVED)

    so = svc.create_sales_order_from_estimate(estimate_document_id=est.document_id, inherit_terms_from_estimate=True)

    # persist to project
    svc.persist_documents_to_project(project_id=project_id, project_workspace_service=psvc, actor="test")
    ...
    docs = TransactionsWorkspaceService.load_serialized_documents_from_project(project_id, psvc)
    ...
    assert any(l.get("source_document_id") == est.document_id for l in lines)
